...
class InputHandler:
    def read(self):
        raise NotImplementedError(f"read not implemented for {self.__name__}")

    def write(self, *args, **kwargs):
        raise NotImplementedError(f"write not implemented for {self.__name__}")


class InputStreamHandler(InputHandler):
...
class InputThreadChannelHandler(InputHandler):
...
def op_input(state, pointer, op, memory, modes):
    ...
    value = state['input'].readline().strip()
    if not value:
        return {'should_eval': False}
...
def run(input, stdin=InputStreamHandler(sys.stdin), stdout=InputStreamHandler(sys.stdout)):
